...
const MyInfoAdmin = (props) => {
  const [changeAdminPasswordQL] = useMutation(CHANGE_ADMIN_PASSWORD, {
    onError: (error) => {
  ...
  const changeAdminPassword = (e) => {
    changeAdminPasswordQL({ variables: { input: form } });
  };
...
export default React.memo(MyInfoAdmin);

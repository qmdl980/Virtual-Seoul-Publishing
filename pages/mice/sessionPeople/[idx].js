...
import SessionPeopleList, {
  SESSION_PEOPLE_QUERY,
  columns,
} from "../../../components/sessionPeople/SessionPeopleList";

const sessionPeopleListDetailPage = ({ idx }) => {
  console.log("idx ::", idx);

  const { loading, error, data, refetch } = useQuery(SESSION_PEOPLE_QUERY, {
  ...
  return (
    <>
      <PcoDocHeader title={"speakerRole"} />
      <div className="wrap">
        <PcoHeader />
        <PcoNavbar />
